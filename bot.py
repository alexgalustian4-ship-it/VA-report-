...
async def get_problems(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    report = (
        ...
        f"➕ Follows : {d['follows']}\n"
        f"👁️ Stories vues : {d['stories']}\n"
        f"❤️ Posts likés : {d['likes']}\n"
        f"💬 Commentaires : {d['comments']}\n"
        f"━━━━━━━━━━━━━━━\n"
        f"⚠️ Problèmes : {d['problems']}\n"
        f"🕐 Envoyé à : {datetime.now().strftime('%H:%M')}"
    )

    kwargs = {"chat_id": REPORT_CHAT_ID, "text": report, "parse_mode": "Markdown"}
    if REPORT_THREAD_ID:
    ...
    for key in ['photo_activity', 'photo_profile']:
        if key in d:
            photo_kwargs = {"chat_id": REPORT_CHAT_ID, "photo": d[key]}
            if REPORT_THREAD_ID:
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("❌ Report annulé. Tape /report pour recommencer.")
    return ConversationHandler.END
